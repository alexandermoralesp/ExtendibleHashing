...
template <typename T>
void BucketLeaf<T>::set_next(long next)
{
    next_ = next;
}

template <typename T>
void BucketLeaf<T>::add_element(T key, long value)
{
    if (size_ == FACTOR_BLOCK)
        throw std::runtime_error("BucketLeaf is full");
...
template <typename T>
void BucketLeaf<T>::remove_element(T key)
{
    if (size_ == 0)
        throw std::runtime_error("BucketLeaf is empty");
    if (elements_->find(key) != elements_->end())
    {
        elements_->erase(key);
        size_--;
...
template <typename T>
long BucketLeaf<T>::get_position(T key)
{
    return (*elements_)[key];
}

template <typename T>
int BucketLeaf<T>::get_size()
{
    return size_;
}
template <typename T>
std::map<T, long> *BucketLeaf<T>::get_elements()
{
    return elements_;
}
template <typename T>
long BucketLeaf<T>::get_next()
{
    return next_;
}

template <typename T>
void BucketLeaf<T>::read_from_file(std::string filename, long position)
{

    std::ifstream file(filename, std::ios::in | std::ios::binary);
    if (!file.is_open())
        throw std::runtime_error("[ERROR]: File not found from BucketLeaf::read_from_file");
...
template <typename T>
void BucketLeaf<T>::write_to_file(std::string filename, long position)
{
    std::ofstream file(filename, std::ios::in | std::ios::out | std::ios::binary);
    if (!file.is_open())
        throw std::runtime_error("[ERROR]: File not found from BucketLeaf::write_to_file");
